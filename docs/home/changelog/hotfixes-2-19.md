...
| RUN-26800 | 2.19.84 | | RUN-26800 | 2.18.99 | Fixed an issue where the `runai list nodes` command in CLI v1 did not display the correct number of free GPUs when using GPU memory. |
...
| RUN-26688 | 2.19.76 | Fixed an issue where node pools could get stuck in the Updating state. |
