...
| ---------------------------- | ---- |
| RUN-22457 | Fixed an issue where in rare edge cases the cluster-sync pod was OOM. |
...
| RUN-22880 | Fixed an issue where the minAvailable field for training-operator CRDs didn't consider all possible replica specs. |
...
| RUN-22623 | Fixed an issue in Openshift where workloads weren't suspended when reaching their idle GPU time limit. |
...
| Internal ID | Description  |
| ---------------------------- | ---- |
| RUN-22452 | Fixed an issue where the scheduler has signature errors if TopologySpreadConstraints is partially defined. |
