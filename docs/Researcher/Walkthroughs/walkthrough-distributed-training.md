...
To run a Horovod-based workload, you will need to run an ``mpirun`` command.

> The command itself is pretty long and might seem intimidating, but you can use the following snippet and just replace the executed command at the end:
...
**NOTE**: If you are using Horovod of version less than or equal to 0.16.4, you can use a simplified ``horovodrun`` command instead of ``mpirun``.
For example:
...
This will provide shell access to the launcher process. From there, you can run your distributed workload using the ``mpirun`` command from above (or ``horovodrun`` if using a Horovod version less than or equal to 0.16.4).
