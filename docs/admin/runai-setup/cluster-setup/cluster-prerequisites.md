...
| BCM  | [NVIDIA Base Command Manager](https://www.nvidia.com/en-us/data-center/base-command/manager/){target=_blank}     | In addition, NVIDIA DGX comes [bundled](dgx-bundle.md) with Run:ai <br><br> Limitation: When configuring the Kubernetes cluster you can only use the default `cluster.local` domain.  |
