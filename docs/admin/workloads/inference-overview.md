...
* Inference workloads can be submitted via Run:ai [user interface](../../admin/workloads/submitting-workloads.md) as well as [Run:ai API](../../developer/cluster-api/workload-overview-dev.md). Internally, spawning an Inference workload also creates a Kubernetes *Service*. The service is an end-point to which clients can connect.
...
* To run Inference from the user interface see [Workloads](../../admin/workloads/submitting-workloads.md).
