...
If the `Enable Node Pools` flag is disabled, all GPU and CPU resources are directly associated with the Department's Quotas. Once an Administrator enables the `Enable Node Pools` flag, all GPU and CPU resources will be included in the `Default` node-pool and summed up to the Department's overall Quotas.
...
__Important best practice:__ As a rule, the sum of the Departments' Quota allocations should be equal to the number of GPUs in the cluster.
