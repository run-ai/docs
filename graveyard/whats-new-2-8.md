...
The Run:ai user interface [requires a URL address](../admin/runai-setup/cluster-setup/cluster-prerequisites.md#domain-name-requirement) to the Kubernetes cluster. The requirement is relevant for SaaS installation only. 
...
In version 2.8,  the default is for the Run:ai administrator to provide a [DNS and a trusted certificate](../admin/runai-setup/cluster-setup/cluster-prerequisites.md#domain-name-requirement). 
